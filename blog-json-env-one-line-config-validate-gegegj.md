...
超大日志不要整文件粘贴，先 [截取片段](https://gegegj.com/blog/json-log-snippet-workflow)。字段里若有 `createdAt: 1716100000`，用 时间戳换算 对照人类时间。语法与业务是两件事：JSON 合法不代表字段名符合接口文档。

含 API Key、数据库密码的配置请先脱敏再粘贴；公共网络慎用任何在线框。更多能力见 JSON 指南、JSON 教程 与 JSON 长文。不支持 JSON5、Schema 校验、转 CSV。
